...
def _build_match_payload(match_id: int, m: dict) -> dict:
    ...
    return {
        "match": {
            ...
            "is_live": bool(m.get("is_live")) or bool(m.get("event_status") == "live"),
            "bzzoiro_id": m.get("bzzoiro_id"),
            "live_data": m.get("live_data"),
...
def get_today_matches(days_ahead: int = Query(default=2, ge=0, le=7)):
    ...
    matches = query(
        ...
        SELECT
            ...
            p1.logo_url AS p1_photo,
            p2.name AS p2_name,
            p2.country_code AS p2_country,
            p2.logo_url AS p2_photo,
            pr1.rtt_score AS p1_rtt,
            pr2.rtt_score AS p2_rtt,
            pr1.momentum AS p1_momentum,
            pr2.momentum AS p2_momentum,
            p1.ranking_movement AS p1_ranking_movement,
            p2.ranking_movement AS p2_ranking_movement,
...
def matches_live():
    ...
    rows = query(
        ...
        SELECT
            ...
            et.tour_category,
            pr1.rtt_score AS p1_rtt, p1.ranking_movement AS p1_ranking_movement,
            pr2.rtt_score AS p2_rtt, p2.ranking_movement AS p2_ranking_movement,

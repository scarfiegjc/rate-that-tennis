...
class LivePredictor:
    ...
    def predict_match(
        ...
        predicted_winner = (
            'first_player'  if prob_p1 > 0.51 else
            'second_player' if prob_p2 > 0.51 else
            None  # genuine 50/50 — no pick
        )
    ...
    def predict_upcoming(self, days_ahead: int = 1, skip_existing: bool = False):
        ...
        # ITF filter: excluded by default until backfill gives players real Elo.
        # Pass --include-itf (or set self.include_itf=True) after running
        # pipeline.itf_backfill to re-enable ITF predictions.
        if getattr(self, 'include_itf', False):
            itf_clause = "1=1"  # include everything
        else:
            itf_clause = "et.tour_category IS NULL OR et.tour_category NOT IN ('ITF', 'Junior')"
...
def main():
    ...
    parser.add_argument('--include-itf', action='store_true',
                        help='Include ITF and Junior matches in predictions. '
                             'Only enable after running pipeline.itf_backfill '
                             'so ITF players have real Elo history. Without the '
                             'backfill, ITF players default to Elo 1500 vs 1500 '
                             'and every match shows 50/50.')
    args = parser.parse_args()

    predictor = LivePredictor(neutralise_rtt=not args.use_rtt)
    predictor.include_itf = getattr(args, 'include_itf', False)
